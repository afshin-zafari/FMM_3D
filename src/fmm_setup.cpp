...
namespace FMM_3D{
  ...
  void read_parameters(int argc,char *argv[]){
    ...
    for(int i=0;i<argc;i++){
      LOG_INFO(LOG_FMM,"CL arg[%d]:%s\n",i,argv[i]);
      if ( strcmp(argv[i],"--data-path") ==0 ){
	LOG_INFO(LOG_FMM,"Data path read  as:%s\n",argv[i+1]);
	strcpy(Parameters.data_path,argv[i+1]);
	LOG_INFO(LOG_FMM,"Data path set as:%s\n",Parameters.data_path);
	continue;
      }
    }
  ...
  void init(int argc, char *argv[]){
    ...
    LOG_INFO(LOG_FMM,"Engine started.\n");
    tree = new Tree;
    import_setup();
    export_setup();
    L_min = 1;
  ...
  void import_setup(){
    string path,p;
    path.assign(Parameters.data_path);
    LOG_INFO(LOG_FMM,"Import from path:%s.\n",path.c_str());
    p=path+"boxes";         import_boxes_binary (p); // This should be the first import to make the Tree
    p=path+"kappas";        import_kappa        (p);
    p=path+"translators";   import_translators  (p);
    p=path+"interpolators"; import_interpolators(p);
    p=path+"Z";             import_Z            (p);
    create_I            ( );
    p=path+"F";             import_F            (p);
    p=path+"R";             import_R            (p);
  ...
  void import_boxes_binary(const string fn){
    ...
    LOG_INFO(LOG_FMM,"Boxes file:%s.\n",fn.c_str());
    FILE *f=fopen(fn.c_str(),"rb");
    if ( f == NULL){
      fprintf(stdout,"Boxes file open failed:%s.\n",fn.c_str());
      return;
    }
    ...
    for(uint32_t i=0;i<L;i++){
      ...
      fprintf(stdout,"Level %d, Boxes:%d.\n",i,nb);
  ...
  void import_translators(const string fn){
    FILE *f=fopen(fn.c_str(),"rb");
    LOG_INFO(LOG_FMM,"XLators file:'%s'.\n",fn.c_str());
    if ( f==NULL) return;
    int L,level;
    fread(&L,sizeof(uint32_t),1,f);
    for(int i =0; i<L-1;i++){
  ...
  void import_interpolators(const string fn){
    FILE *f=fopen(fn.c_str(),"rb");
    LOG_INFO(LOG_FMM,"inerpolator file:'%s'.\n",fn.c_str());
    if ( f==NULL) return;
    uint32_t N,level,n;
